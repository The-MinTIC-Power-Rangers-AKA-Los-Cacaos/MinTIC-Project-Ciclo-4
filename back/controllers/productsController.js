...
exports.getProducts = catchAsyncErrors(async (req, res, next) => {
    ...
    let filteredProductsCount = prod.length;
...
exports.createUpdateProductReview = catchAsyncErrors(async (req, res, next) => {
    ...
    const isReviewed = product.opinions.find(item =>
    ...
    product.rate = product.opinions.reduce((acc, opinion) =>
        opinion.rating + acc, 0) / product.opinions.length
...
exports.deleteProductReview = catchAsyncErrors(async (req, res, next) => {
    ...
    const opinions = product.opinions.filter(opinion =>
        opinion._id.toString() !== req.query.reviewId.toString());

    const totalRatings = opinions.length;

    const rate = product.opinions.reduce((acc, Opinion) =>
        Opinion.rating + acc, 0) / opinions.length;
